...
class User(Kontext):
    ...
    def loginx(self, request):
        ...
        with plugins.runtime.AUTH as auth:
            self._request.ctx.session['user'] = auth.validate_user(self._plugin_ctx, None, None)
    ...
    def login(self, request):
        ...
        with plugins.runtime.AUTH as auth:
            ans = {}
            self._request.ctx.session['user'] = auth.validate_user(
                self._plugin_ctx, request.form['username'], request.form['password'])
            if not auth.is_anonymous(self._request.ctx.session['user'].get('id', None)):
                if request.args.get('return_url', None):
                    self.redirect(request.args.get('return_url'))
                else:
    ...
    def logoutx(self, request):
        self.disabled_menu_items = USER_ACTIONS_DISABLED_ITEMS
        plugins.runtime.AUTH.instance.logout(self._request.ctx.session)
        self.init_session()
        self.refresh_session_id()
