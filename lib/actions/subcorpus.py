...
class Subcorpus(Querying):
    ...
    def _create_subcorpus(self, request):
        ...
        if raw_cql:
            ...
            imp_cql = (full_cql,)
        elif len(aligned_corpora) > 0 and plugins.has_plugin('live_attributes'):
            if corpus_info.metadata.label_attr and corpus_info.metadata.id_attr:
                within_cql = None
                attrs = json.loads(request.form.get('attrs', '{}'))
                sel_match = plugins.get('live_attributes').get_attr_values(self._plugin_api, corpus=self.corp,
                                                                           attr_map=attrs,
