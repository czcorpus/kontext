...
class WriteBackend(DatabaseWriteBackend):
    ...
    async def remove_corpus(self, corpus_id):
    ...
    async def _create_structattr_if_none(self, corpus_id, name):
        """
    ...
    async def _find_article(self, contents):
        """
    ...
    async def _create_struct_if_none(self, corpus_id, name):
        """
    ...
    async def save_corpus_config(self, install_json, registry_conf, corp_size):
        ...
        async with self._db.cursor() as cursor:
            ...
            if install_json.reference.default:
                def_art_id = await self._find_article(install_json.reference.default)
            ...
            if install_json.reference.other_bibliography:
                other_art_id = await self._find_article(install_json.reference.other_bibliography)
            ...
            for art in install_json.reference.articles:
                std_art_id = await self._find_article(art)
            ...
            await self._create_struct_if_none(install_json.ident, install_json.sentence_struct)
    ...
    async def update_corpus_config(self, install_json, registry_conf, corp_size):
        t1 = datetime.datetime.now(
            tz=pytz.timezone('Europe/Prague')).strftime("%Y-%m-%dT%H:%M:%S%z")
        ...
        async with self._db.cursor() as cursor:
            ...
            for prop, structattr in await self._find_structattr_use(install_json.ident).items():
    ...
    async def save_corpus_article(self, text):
        async with self._db.cursor() as cursor:
    ...
    async def attach_corpus_article(self, corpus_id, article_id, role):
        async with self._db.cursor() as cursor:
            await cursor.execute(
                'INSERT INTO kontext_corpus_article (corpus_name, article_id, role) '
                'VALUES (%s, %s, %s)', (corpus_id, article_id, role))

    async def _registry_table_exists(self, corpus_id):
        async with self._db.cursor(buffered=True) as cursor:
            cursor.execute(
                'SELECT COUNT(*) AS cnt FROM registry_conf WHERE corpus_name = %s LIMIT 1', (corpus_id,))
            row = await cursor.fetchone()
            return row['cnt'] == 1 if row else False

    async def _registry_variable_exists(self, corpus_id, variant):
        async with self._db.cursor(buffered=True) as cursor:
    ...
    async def save_registry_table(self, corpus_id, variant, values):
        values = dict(values)
        await self._create_struct_if_none(corpus_id, values.get('DOCSTRUCTURE', None))
        async with self._db.cursor() as cursor:
            t1 = datetime.datetime.now(
                tz=pytz.timezone('Europe/Prague')).strftime("%Y-%m-%dT%H:%M:%S%z")

            if await self._registry_table_exists(corpus_id):
            ...
            if await self._registry_variable_exists(corpus_id, variant):
                if variant is not None:
                    await cursor.execute(
    ...
    async def update_corpus_posattr_references(self, corpus_id, posattr_id, fromattr_id, mapto_id):
        """
    ...
    async def save_corpus_alignments(self, corpus_id, aligned_ids):
        async with self._db.cursor() as cursor:
    ...
    async def _structattr_exists(self, corpus_id, struct_id, name):
        async with self._db.cursor() as cursor:
            await cursor.execute(
                'SELECT COUNT(*) AS cnt FROM corpus_structattr '
                'WHERE corpus_name = %s AND structure_name = %s AND name = %s',
    ...
    async def save_corpus_structattr(self, corpus_id, struct_id, name, position, values):
        """
        """
        if await self._structattr_exists(corpus_id, struct_id, name):
    ...
    async def save_subcorpattr(self, corpus_id, struct_name, attr_name, idx):
        async with self._db.cursor() as cursor:
            await cursor.execute(
    ...
    async def save_freqttattr(self, corpus_id, struct_name, attr_name, idx):
        async with self._db.cursor() as cursor:
            await cursor.execute(
                'UPDATE corpus_structattr SET freqttattrs_idx = %s '
                'WHERE corpus_name = %s AND structure_name = %s AND name = %s', (idx, corpus_id, struct_name, attr_name))

    async def _find_structures_use(self, corp_id: str):
        async with self._db.cursor() as cursor:
            await cursor.execute(
    ...
    async def _find_structattr_use(self, corp_id: str):
        async with self._db.cursor() as cursor:
            await cursor.execute(
                'SELECT '
