...
class DefaultAuthHandler(AbstractInternalAuth):
    ...
    def __init__(self, db, sessions, anonymous_user_id, corplist, login_url, logout_url):
        """
        """
    ...
    def get_login_url(self, return_url=None):
        if return_url is not None:
            return '%s?continue=%s' % (self.login_url, urllib.quote(return_url))
        else:
            return self.login_url

    def get_logout_url(self, return_url=None):
        if return_url is not None:
            return '%s?continue=%s' % (self.logout_url, urllib.quote(return_url))
        else:
...
def _load_corplist(conf):
    corptree_path = conf.get('plugins', 'auth')['lindat:corptree_path']
...
def create_instance(conf, db, sessions):
    ...
    return DefaultAuthHandler(db=db, sessions=sessions,
                              anonymous_user_id=int(plugin_conf['anonymous_user_id']),
                              corplist=_load_corplist(conf),
                              login_url=plugin_conf['login_url'],
                              logout_url=plugin_conf['logout_url'])
