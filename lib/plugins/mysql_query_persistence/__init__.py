...
class MySqlQueryPersistence(AbstractQueryPersistence):
    ...
    async def update_preflight_stats(
            self, plugin_ctx, preflight_id, corpus, subc_id, estimated_size, actual_size):
        async with self._archive.cursor() as cursor:
            await cursor.execute(
                'INSERT INTO kontext_preflight_stats (id, corpus_name, subc_id, estimated_size, actual_size) '
                'VALUES (%s, %s, %s, %s, %s) '
                'ON DUPLICATE KEY UPDATE '
                'estimated_size = IFNULL(%s, estimated_size), '
                'actual_size = IFNULL(%s, actual_size) ',
                (preflight_id, corpus, subc_id, estimated_size, actual_size, estimated_size, actual_size))
