...
def create_instance(settings, ticket_id_provider):
    ...
    css_url1 = settings.get('plugins', 'appbar').get('css_url1', None)
    ...
    return AppBar(root_url=root_url, ticket_id_provider=ticket_id_provider, server=server, path=path, port=port,
                  css_url=css_url, css_url1=css_url1, css_url_ie=css_url_ie, js_url=js_url)


class AppBar(object):

    def __init__(self, ticket_id_provider, root_url, server, path, port, css_url, css_url1, css_url_ie, js_url):
