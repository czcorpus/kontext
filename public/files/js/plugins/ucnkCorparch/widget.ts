...
import * as commonDefault from '../defaultCorparch/common';
import * as popupBox from '../../popupbox';
import {CorplistTableStore} from './corplist';
...
class FavoritesTab implements WidgetTab, Kontext.ICorpusSwitchAware<CorpusSwitchFavData> {
    ...
    csGetStateKey():string {
        return 'ucnk-corparch-corpus-switch-fav-data';
    }
    ...
    init():void {
        ...
        function createItem(data:common.CorplistItemUcnk):HTMLElement {
...
class StarComponent {
     ...
     * @param pageModel
     */
    constructor(favoriteItemsTab:FavoritesTab, pageModel:Kontext.PluginApi, querySetupHandler:Kontext.QuerySetupHandler,
            editable:boolean) {
        ...
        const currItem:common.CorplistItemUcnk = this.extractItemFromPage();
    ...
    private inferItemCore(corpus_id:string, subcorpus_id:string,
        ...
        let ans:common.CorplistItemUcnk;
    ...
    extractItemFromPage(userItemFlag?:commonDefault.Favorite):common.CorplistItemUcnk {
        ...
        let alignedCorpora:Array<string> = [];
        let item:common.CorplistItemUcnk;
        ...
        if (this.editable) {
            $(this.starImg).on('click', (e) => {
                if (!this.starSwitch.isStarred()) {
                    this.setFavorite(commonDefault.Favorite.FAVORITE);

                } else {
                    this.setFavorite(commonDefault.Favorite.NOT_FAVORITE);
                }
            });
...
export class Corplist implements CorparchCommon.Widget {
    ...
    private buildWidget() {
        ...
        const favData:Array<common.CorplistItemUcnk> = pluginData['favorite'] || [];
