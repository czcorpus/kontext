...
export interface TextTypesModelState {
     ...
     */
    selectionHistory:Array<Array<TextTypes.AttributeSelection>>;
...
export class TextTypesModel extends StatefulModel<TextTypesModelState>
    ...
    constructor(dispatcher:IFullActionControl, pluginApi:IPluginApi, data:InitialData,
        ...
        super(
            dispatcher,
            {
                ...
                selectAll: pipe(
                    attributes,
                    List.map(
                        (item:TextTypes.AttributeSelection) => tuple(item.name, false)
                    ),
                    Dict.fromEntries()
                ),
                metaInfo: {},
        ...
        this.addActionHandler<Actions.ExtendedInformationRequest>(
            ...
            action => {
                this.changeState(state => {
                    state.isBusy = true;
                    const attr = this.getAttribute(state, action.payload.attrName);
                    const ident = action.payload.ident;
                    const attrIdx = state.attributes.indexOf(attr);
                    const srchIdx = attr.getValues().findIndex(v => v.ident === ident);
                    if (srchIdx > - 1 && attr.getValues()[srchIdx].numGrouped < 2) {
                        state.attributes[attrIdx] = attr.mapValues(item => ({
                            ...item,
                        ...
                        const message = this.pluginApi.translate(
                            ...
                            {num_items: attr.getValues()[srchIdx].numGrouped}
                        );
        ...
        this.addActionHandler<Actions.SnapshotState>(
            ActionName.SnapshotState,
            action => {
                this.changeState(state => {
                    state.selectionHistory.push(state.attributes);
                });
            }
        );
        ...
        this.addActionHandler<Actions.FilterWholeSelection>(
            ...
            action => {
                this.changeState(state => {
                    this.filterWholeSelection(state, action.payload.data);
                });
            }
        );
    ...
    private resetAutoComplete(state:TextTypesModelState, attrName:string):void {
        ...
        if (attr) {
            ...
            this.state.attributes[idx] = attr.resetAutoComplete();
        }
    }
    ...
    private handleAttrTextInputChange(state:TextTypesModelState, attrName:string, value:string) {
        ...
        if (attr) {
            ...
            state.attributes[idx] = attr.setTextFieldValue(value);
        }
    }
    ...
    private setTextInputAttrValue(state:TextTypesModelState, attrName:string, ident:string, label:string,
        ...
        state.attributes[idx] = updatedAttr;
    }
    ...
    private changeValueSelection(state:TextTypesModelState, attrIdent:string, itemIdx:number):void {
        const attr = this.getAttribute(state, attrIdent);
        const idx = state.attributes.indexOf(attr);
        if (attr) {
            state.attributes[idx] = attr.toggleValueSelection(itemIdx);
    ...
    private applySelectAll(state:TextTypesModelState, ident:string) {
        const item = this.getAttribute(state, ident);
        const idx = state.attributes.indexOf(item);
        if (item.containsFullList()) {
            state.selectAll[ident] = !state.selectAll[ident];
            const newVal = state.selectAll[ident];
            state.attributes[idx] = item.mapValues((item) => ({
                ...item,
                selected: newVal,
            }));
            state.hasSelectedItems = this.findHasSelectedItems(state);
    ...
    getAttribute(state:TextTypesModelState, ident:string):TextTypes.AttributeSelection {
    ...
    private _getAttributes(state:TextTypesModelState):Array<TextTypes.AttributeSelection> {
    ...
    private filterWholeSelection(state:TextTypesModelState, filterData:SelectionFilterMap) {
        ...
        for (k in filterData) {
            this.updateItems(state, k, filterData[k].map(v => v.ident));
            this.mapItems(
                state,
                k,
                (v, i) => {
    ...
    private updateItems(state:TextTypesModelState, attrName:string, items:Array<string>):void {
        const attr = this.getAttribute(state, attrName);
        const idx = state.attributes.indexOf(attr);
        if (idx > -1) {
            state.attributes[idx] = attr.updateItems(items);
        }
    }
    ...
    filter(state:TextTypesModelState, attrName:string, fn:(v:TextTypes.AttributeValue)=>boolean):void {
        ...
        const idx = state.attributes.indexOf(attr);
        if (idx > -1) {
            state.attributes[idx] = attr.filter(fn);
        }
    }

    mapItems(state:TextTypesModelState, attrName:string, mapFn:(v:TextTypes.AttributeValue,
            i?:number)=>TextTypes.AttributeValue):void {
        const attr = this.getAttribute(state, attrName);
        const idx = state.attributes.indexOf(attr);
        if (idx > -1) {
            const newAttr = attr.mapValues(mapFn);
            state.attributes[idx] = newAttr;
        }
    }

    setValues(state:TextTypesModelState, attrName:string, values:Array<string>):void {
        const attr = this.getAttribute(state, attrName);
        const idx = state.attributes.indexOf(attr);
        ...
        if (idx > -1) {
            state.attributes[idx] = attr.setValues(values2);
    ...
    setAutoComplete(state:TextTypesModelState, attrName:string, values:Array<TextTypes.AutoCompleteItem>):void {
        ...
        if (attr) {
            const idx = state.attributes.indexOf(attr);
            state.attributes[idx] = attr.setAutoComplete(values);
        }
    }
    ...
    private findHasSelectedItems(state:TextTypesModelState, attrName?:string):boolean {
        if (attrName !== undefined) {
            ...
            return this._getAttributes(state).some(item => item.hasUserChanges());
        }
    }
    ...
    getAttributesWithSelectedItems(state:TextTypesModelState, includeLocked:boolean):Array<string> {
        return pipe(
            state.attributes,
            List.filter(
    ...
    setExtendedInfo(state:TextTypesModelState, attrName:string, ident:string, data:{[key:string]:string|number}):void {
        ...
        if (attrName) {
            ...
            this.state.attributes[attrIdx] = attr.setExtendedInfo(ident, data);
