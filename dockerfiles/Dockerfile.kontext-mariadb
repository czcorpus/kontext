...
RUN cat /tmp/mariadb-init.sql /tmp/schema_corparch.sql /tmp/schema_auth.sql /tmp/schema_query_persistence.sql /tmp/user_items.sql /tmp/query_history.sql /tmp/integration_db.sql > /docker-entrypoint-initdb.d/init.sql
RUN rm -f /tmp/*.sql
