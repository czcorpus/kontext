...
class DbTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.sd.commit()
        async with self.s.connection() as s_db:
            await s_db.execute(DROP_DATA_TABLE_SQL)
            await s_db.execute(CREATE_DATA_TABLE_SQL)
            await s_db.commit()
    ...
    async def test_set_and_get(self):
        ...
        out_r = await self.r.get(key)
        out_s = await self.s.get(key)
        self.assertEqual(out_r, out_s)

    async def test_list_get_and_list_append(self):
        """
        ...
        for i in range(0, length):
            await self.r.list_append(key, i)
            await self.s.list_append(key, i)
            check_list.append(i)
        out_r = await self.r.list_get(key)
        out_s = await self.s.list_get(key)
        self.assertTrue(out_r == out_s == check_list)

    async def test_list_get_with_range(self):
        """
        test the list_append and list_get methods
        in case of error: verify that the "key" column is set as PRIMARY KEY
        ...
        for pair in ranges:
            from_idx = pair[0]
            to_idx = pair[1]
            out_r = await self.r.list_get(key, from_idx, to_idx)
            out_s = await self.s.list_get(key, from_idx, to_idx)
            if to_idx < 0:
    ...
    async def test_list_len(self):
        ...
        for i in range(0, length):
            await self.r.list_append(key, i)
            await self.s.list_append(key, i)
        out_r = await self.r.list_len(key)
        out_s = await self.s.list_len(key)
    ...
    async def test_list_pop(self):
        ...
        for i in range(0, length):
            val = {'q': 'value' + str(i)}
            await self.r.list_append(key, val)
            await self.s.list_append(key, val)
            checklist.append(val)
        val_r = await self.r.list_pop(key)
        val_s = await self.s.list_pop(key)
    ...
    async def test_list_set(self):
        ...
        set_position = 3
        set_value = 'check'
        await self.r.list_set(key, set_position, set_value)
        await self.s.list_set(key, set_position, set_value)
        checklist[set_position] = set_value
        out_r = await self.r.list_get(key)
        out_s = await self.s.list_get(key)
        self.assertTrue(out_r == out_s == checklist)

    async def test_list_trim_with_range(self):
        """
        ...
        for pair in ranges:
            key = 'list'
            length = 10
            await self.r.remove(key)
            await self.s.remove(key)
            checklist = []
            ...
            keep_left = pair[0]
            keep_right = pair[1]
            await self.r.list_trim(key, keep_left, keep_right)
            await self.s.list_trim(key, keep_left, keep_right)
            out_r = await self.r.list_get(key)
            out_s = await self.s.list_get(key)
    ...
    async def test_hash_set_and_hash_get(self):
        """
        ...
        field = 'field'
        value = 'bar'
        await self.r.hash_set(key, field, value)
        await self.s.hash_set(key, field, value)
        out_r = await self.r.hash_get(key, field)
        out_s = await self.s.hash_get(key, field)
        self.assertEqual(out_r, out_s)

    async def test_hash_del(self):
        """
        ...
        absent = 'f4'
        # create hash records:
        await self.r.hash_set(key, field1, value1)
        await self.s.hash_set(key, field1, value1)
        await self.r.hash_set(key, field2, value2)
        await self.s.hash_set(key, field2, value2)
        await self.r.hash_set(key, field3, value3)
        await self.s.hash_set(key, field3, value3)
        # and delete them:
        await self.r.hash_del(key, field1)
        await self.s.hash_del(key, field1)
        # try to delete a non-existing field, must not throw error (to simulate redis-like behaviour)
        await self.r.hash_del(key, absent)
        await self.s.hash_del(key, absent)
        out_r = await self.r.hash_get_all(key)
        out_s = await self.s.hash_get_all(key)
        if VERBOSE:
            print('\nTesting hash_del:')
            print(('redis: {0}, sqlite: {1}'.format(out_r, out_s)))
        self.assertEqual(out_r, out_s)

    async def test_hash_get_all(self):
        """
    ...
    async def test_rename(self):
        ...
        new_key = 'new'
        value = 'val'
        await self.r.set(old_key, value)
        await self.s.set(old_key, value)
        await self.r.rename(old_key, new_key)
        await self.s.rename(old_key, new_key)
        out_r = await self.r.get(new_key)
        out_s = await self.s.get(new_key)
        self.assertEqual(out_r, out_s)
    ...
    async def test_exists(self):
        ...
        await self.r.set(key, value)
        await self.s.set(key, value)
        out_r = await self.r.exists(key)
        out_s = await self.s.exists(key)
        self.assertEqual(out_r, out_s)
    ...
    async def test_set_ttl(self):
        ...
        if TEST_TTL_METHODS:
            ...
            await self.r.set(key, value)
            await self.s.set(key, value)
            await self.r.set_ttl(key, 2)
            await self.s.set_ttl(key, 2)
            time.sleep(1)
            out_r = [await self.r.exists(key)]
            out_s = [await self.s.exists(key)]
            time.sleep(1)
            out_r.append(await self.r.exists(key))
            out_s.append(await self.s.exists(key))
            if VERBOSE:
                print('testing set_ttl:')
                print(('redis: {0}, sqlite: {1}'.format(out_r, out_s)))
            self.assertTrue(out_r == out_s == [True, False])

    async def test_clear_ttl(self):
        """
        ...
        if TEST_TTL_METHODS:
            ...
            out_r = [await self.r.exists(key)]
            out_s = [await self.s.exists(key)]
            await self.r.clear_ttl(key)
            await self.s.clear_ttl(key)
            time.sleep(1)
            out_r.append(await self.r.exists(key))
            out_s.append(await self.s.exists(key))
            if VERBOSE:
    ...
    async def test_get_ttl(self):
        if TEST_TTL_METHODS:
            ...
            await self.r.set(key, value)
            await self.s.set(key, value)
            await self.r.set_ttl(key, 5)
            await self.s.set_ttl(key, 5)

            t1, t2 = await self.r.get_ttl(key), await self.s.get_ttl(key)
            self.assertTrue(t1 > 0)
            self.assertTrue(t2 > 0)

    async def test_get_ttl_initial(self):
        if TEST_TTL_METHODS:
            key = 'foo'
            value = 'bar'
            await self.r.set(key, value)
    ...
    async def test_incr(self):
        ...
        await self.r.set(key1, number1)
        r_out1 = await self.r.incr(key1, amount)
        r_out2 = await self.r.incr(key2, amount)

        await self.s.set(key1, number1)
        s_out1 = await self.s.incr(key1, amount)
        s_out2 = await self.s.incr(key2, amount)
    ...
    async def test_hash_set_map(self):
        """
        ...
        await self.r.hash_set_map('hash1', d)
        await self.s.hash_set_map('hash1', d)
        out_r = str(await self.r.hash_get('hash1', 'val1')) + await self.r.hash_get('hash1', 'val2')
        out_s = str(await self.s.hash_get('hash1', 'val1')) + await self.s.hash_get('hash1', 'val2')
    ...
    async def test_get_instance(self):
        ...
        """
        self.assertTrue(isinstance(await self.r.get_instance(1), KeyValueStorage))
        self.assertTrue(isinstance(await self.s.get_instance(1), KeyValueStorage))
